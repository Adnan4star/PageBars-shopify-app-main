...
class DashboardController extends Controller
{
    ...
    public function PageBar(Request $request){
        try {
            ...
            $page_bars=PageBar::select('id','bar_name','leads','is_active','type','type_ids')->where('shop_id',$shop->id)->orderBy('id','desc')->get();

            $bar_data=[];
            foreach ($page_bars as $page_bar){
                $data['id']=$page_bar->id;
                $data['bar_name']=$page_bar->bar_name;
                $data['leads']=$page_bar->leads;
                $data['is_active']=$page_bar->is_active;
                if($page_bar->type=='blogs'){
                    $blog_ids=explode(',',$page_bar->type_ids);
                    $blog=Blog::where('id',$blog_ids[0])->first();
                    $blog_article=BlogArticle::where('blog_id',$blog->id)->first();
                    if($blog_article && $blog_article->handle) {
                        $data['url'] = 'https://' . $shop->shop . '/blogs/' . $blog->handle . '/' . $blog_article->handle;
                    }
                }elseif($page_bar->type=='products'){
                    $product_ids=explode(',',$page_bar->type_ids);
                    $product=Product::where('id',$product_ids[0])->first();
                    if($product && $product->handle) {
                        $data['url'] = 'https://' . $shop->shop . '/products/' . $product->handle;
                    }

                }elseif($page_bar->type=='collections'){
                    $collection_ids=explode(',',$page_bar->type_ids);
                    $collection=Collection::where('id',$collection_ids[0])->first();
                    if($collection && $collection->handle) {
                        $data['url'] = 'https://' . $shop->shop . '/collections/' . $collection->handle;
                    }
                }else{
                    $data['url']='';
                }
...
public function PageBarView(Request $request){
    ...
    public function PageBarDetail(Request $request){
        ...
        try {
            $shop = getShop($request->get('shopifySession'));
            $page_bar_details = Order::select('customer_id', 'first_name', 'last_name', DB::raw('SUM(total_price) as total_price'), DB::raw('COUNT(*) as order_count'), 'currency','phone')
                ->where('page_bar_id', $request->id)
                ->where('shop_id', $shop->id)
                ->groupBy('customer_id', 'first_name', 'last_name', 'currency','phone')
                ->get();
    ...
    public function OldSaveData(Request $request){
...
//            $page_bar->bar_button_enabled=$request->bar_button_enabled;
            ...
            if ($request->hasFile('bar_image')) {
                $file = $request->file('bar_image');
                $destinationPath = 'images/';
                $filename1 = now()->format('YmdHi') . str_replace([' ', '(', ')'], '-', $file->getClientOriginalName());
                $file->move($destinationPath, $filename1);
                $filename1 = (asset('images/' . $filename1));
                $page_bar->bar_image=$filename1;
            }
...
//            $page_bar->bar_title_text_weight=$request->bar_title_text_weight;
            ...
            $page_bar->mobile_bar_title_text_size=$request->mobile_bar_title_text_size;
            $page_bar->mobile_bar_button_text_size=$request->mobile_bar_button_text_size;


            $page_bar->bar_button_radius=$request->bar_button_radius;
            $page_bar->discount_code_status=$request->discount_status;
            $page_bar->save();
            ...
            if ($request->hasFile('mobile_form_image')) {
                $file = $request->file('mobile_form_image');
                $destinationPath = 'images/';
                $filename2 = now()->format('YmdHi') . str_replace([' ', '(', ')'], '-', $file->getClientOriginalName());
                $file->move($destinationPath, $filename2);
                $filename2 = (asset('images/' . $filename2));
                $page_bar_form->mobile_form_image=$filename2;
            }
...
//            $page_bar_form->form_secondary_button_text_color=$request->form_secondary_button_text_color;
            $page_bar_form->form_background_color=$request->form_background_color;
            $page_bar_form->form_fields_color=$request->form_fields_color;
            $page_bar_form->form_close_button_color=$request->form_close_button_color;
            $page_bar_form->form_title_size=$request->form_title_size;
//            $page_bar_form->form_title_weight=$request->form_title_weight;
            $page_bar_form->form_sub_title_size=$request->form_sub_title_size;
//            $page_bar_form->form_sub_title_weight=$request->form_sub_title_weight;
            $page_bar_form->form_button_text_size=$request->form_button_text_size;
//            $page_bar_form->form_button_text_weight=$request->form_button_text_weight;
            $page_bar_form->form_button_pop_up_radius=$request->form_button_pop_up_radius;
            $page_bar_form->form_button_fields_radius=$request->form_button_fields_radius;
            $page_bar_form->form_button_button_radius=$request->form_button_button_radius;


            $page_bar_form->mobile_form_title_size=$request->mobile_form_title_size;
            $page_bar_form->mobile_form_sub_title_size=$request->mobile_form_sub_title_size;
            $page_bar_form->mobile_button_text_size=$request->mobile_button_text_size;
            ...
            $page_bar_success_form->success_form_button_link=$request->success_form_button_link;
            $page_bar_success_form->success_form_title_color=$request->success_form_title_color;
            $page_bar_success_form->success_form_button_color=$request->success_form_button_color;
            $page_bar_success_form->success_form_button_text_color=$request->success_form_button_text_color;
            $page_bar_success_form->success_form_background_color=$request->success_form_background_color;
            $page_bar_success_form->success_form_middle_title_color=$request->success_form_middle_title_color;
            $page_bar_success_form->success_form_close_button_color=$request->success_form_close_button_color;
            $page_bar_success_form->success_form_title_size=$request->success_form_title_size;
            $page_bar_success_form->success_form_middle_text_size=$request->success_form_middle_text_size;
            $page_bar_success_form->success_form_button_size=$request->success_form_button_size;
            $page_bar_success_form->success_form_popup_radius=$request->success_form_popup_radius;
            $page_bar_success_form->success_form_button_radius=$request->success_form_button_radius;

            $page_bar_success_form->mobile_success_form_title_size=$request->mobile_success_form_title_size;
            $page_bar_success_form->mobile_success_form_middle_text_size=$request->mobile_success_form_middle_text_size;
            $page_bar_success_form->mobile_success_form_button_size=$request->mobile_success_form_button_size;
            $page_bar_success_form->save();
...
//            }
    ...
    public function oldUpdateData(Request $request){
        try {
            $shop = getShop($request->get('shopifySession'));
            $client = new Rest($shop->shop, $shop->access_token);
            if($request->id) {
...
//                    $page_bar_form->form_button_text_weight = $request->form_button_text_weight;
    ...
    public function oldDeleteData(Request $request){
        try {
            $shop = getShop($request->get('shopifySession'));
            $client = new Rest($shop->shop, $shop->access_token);
            $page_bar=PageBar::find($request->id);
    ...
    public function CreateUpdateMetafield($session){
        ...
        foreach ($page_bars as $page_bar){
            ...
            elseif ($page_bar->type=='blogs'){
                $data=BlogArticle::whereIn('id',$type_ids)->pluck('handle')->toArray();
            }
            ...
            $getdata['bar_button_radius']=$page_bar->bar_button_radius;

            $getdata['bar_image']=$page_bar->bar_image;


            $getdata['mobile_bar_title_text_size']=$page_bar->mobile_bar_title_text_size;
            $getdata['mobile_bar_button_text_size']=$page_bar->mobile_bar_button_text_size;
            ...
            $getdata['mobile_form_image']=$page_bar_form->mobile_form_image;
            ...
            $getdata['mobile_form_title_size']=$page_bar_form->mobile_form_title_size;
            $getdata['form_fields_color']=$page_bar_form->form_fields_color;
            $getdata['form_button_pop_up_radius']=$page_bar_form->form_button_pop_up_radius;
            $getdata['form_button_fields_radius']=$page_bar_form->form_button_fields_radius;
            $getdata['form_button_button_radius']=$page_bar_form->form_button_button_radius;

            $getdata['form_title_weight']=$page_bar_form->form_title_weight;
            $getdata['form_sub_title_size']=$page_bar_form->form_sub_title_size;
            $getdata['mobile_form_sub_title_size']=$page_bar_form->mobile_form_sub_title_size;
            ...
            $getdata['mobile_button_text_size']=$page_bar_form->mobile_button_text_size;
            ...
            $getdata['success_form_button_color']=$page_bar_success_form->success_form_button_color;
            $getdata['success_form_close_button_color']=$page_bar_success_form->success_form_close_button_color;
            $getdata['success_form_title_size']=$page_bar_success_form->success_form_title_size;
            $getdata['success_form_middle_text_size']=$page_bar_success_form->success_form_middle_text_size;
            $getdata['success_form_button_size']=$page_bar_success_form->success_form_button_size;
            $getdata['success_form_popup_radius']=$page_bar_success_form->success_form_popup_radius;
            $getdata['success_form_button_radius']=$page_bar_success_form->success_form_button_radius;

            $getdata['mobile_success_form_title_size']=$page_bar_success_form->mobile_success_form_title_size;
            $getdata['mobile_success_form_middle_text_size']=$page_bar_success_form->mobile_success_form_middle_text_size;
            $getdata['mobile_success_form_button_size']=$page_bar_success_form->mobile_success_form_button_size;

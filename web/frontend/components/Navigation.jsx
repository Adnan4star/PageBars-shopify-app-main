...
// import eBookUrl from '../assets/eBook.svg';
...
const Navigation = () => {
  ...
  useEffect(() => {
    if(currentPlan){
      if(currentPlan.contacts_count >= currentPlan.plan.contacts_count){
        setIsModalOpen(true);
  ...
  // bar content state
  const [title, setTitle] = useState('Would you like to Get this ebook for free?');
  const [scroll, setScroll] = useState('');
  const [btnText, setBtnText] = useState('Get eBook');
  ...
  const [closeBtnColor, setCloseBtnColor] = useState('#F1F1F1');
  ...
  useEffect(() => {
    setBarFile("https://phpstack-1219737-4337556.cloudwaysapps.com/web/frontend/assets/eBook.svg");
  }, []);

  /// form content
  const [formTitle, setFormTitle] = useState('Fill in your information to get the eBook.');
  const [formSubtitle, setFormSubtitle] = useState('Make sure the information is correct.');
  const [formPrimaryBtnText, setFormPrimaryBtnText] = useState('Submit');
  ...
  const [formWarningText, setFormWarningText] = useState('By signing up, you agree to receive marketing emails. View our privacy policy and terms of service for more info.');
  const [] = useState('');
  ...
  const [formBtnColor, setFormBtnColor] = useState('#F1F1F1');
  ...
  const [formBgColor, setFormBgColor] = useState('#333333');
  const [formCloseBtnColor, setFormCloseBtnColor] = useState('#F1F1F1');
  ...
  const tabs = [
    {
      id: 'home', content: 'Information', accessibilityLabel: 'All customers', panelID: 'home-content', render:
        <Information
          barName={barName}
    ...
    {
      id: 'Bar', content: 'Bar', panelID: 'bar-content', render:
        <BarContent
          title={title}
          scroll={scroll}
          btnText={btnText}
    ...
    {
      id: 'Form', content: 'Form', panelID: 'form-content', render:
        <FormContent
    ...
    {
      id: 'Success', content: 'Success', panelID: 'success-content', render:
        <SuccessForm
  ...
  const handleCreateBar = async () => {
    const data = {
      ...
      // discount_code_status: discountStatus ? "1" : "0",
  ...
  return (
    <>
      <Page title="Page Bars" primaryAction={<Button disabled={isLimitReached} variant="primary" onClick={handleNextButtonClick} loading={btnLoading} >{buttonText}</Button>} secondaryActions={<Button>Save Draft</Button>} fullWidth>
        <Layout>
          <Layout.Section>
            <div className='Navigation'>
              <Card padding={'0'}>
                <Tabs tabs={tabs} selected={selected} onSelect={handleTabChange} fitted style={{}} />
              </Card>
            </div>
